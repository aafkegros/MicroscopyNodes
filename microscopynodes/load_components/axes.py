...
class Axes(MiNObject):
    ...
    def set_data(self, dataset_model):
        ...
        self.node_group.nodes[f"[Microscopy Nodes scale]"].vector = dataset_model.scale
        self.node_group.nodes[f"[Microscopy Nodes pixel_size]"].vector = tuple(np.diag(dataset_model.affine))
        return
    ...
    def init_gn(self):
        ...
        node_group.interface.new_socket(name=f"unit per tick", in_out="INPUT",socket_type='NodeSocketFloat')
