...
class Axes(MiNObject):
    ...
    def init_obj(self): 
        # instantiate new blender object
        super().init_obj()
    ...
    def set_data(self, dataset_model):
        ...
        # set input values
        # self.object.modifiers[-1][self.node_group.interface.items_tree[0].identifier] = tick_um
        # self.object.modifiers[-1][self.node_group.interface.items_tree[1].identifier] = True
        # TODO fix unit setting - bboxing is also wrong!!
        ...
        # self.node_group.nodes[f"[Microscopy Nodes size_px]"].vector = (dataset_model.intrinsic_bbox[1], dataset_model.intrinsic_bbox[3], dataset_model.intrinsic_bbox[5])
        # self.node_group.nodes[f"[Microscopy Nodes scale]"].vector = (dataset_model.scale, dataset_model.scale, dataset_model.scale)
        # self.node_group.nodes[f"[Microscopy Nodes pixel_size]"].vector = tuple(np.diag(dataset_model.channels[0].affine)[:-1])
        return

    def ensure_links_of_objects(dataset):
        self.node_group.nodes.get("Holder Scale").default_value = dataset.holder
    ...
    def init_gn(self):
        ...
        selfinfo.inputs[0].default_value = self.object
        selfinfo.location = (-1050, -100)

        containerinfo = nodes.new('GeometryNodeObjectInfo')
        containerinfo.name = "Holder Scale"
        # containerinfo.inputs[0].default_value = container
        ...
        self.node_group.interface.new_socket(name=f"unit per tick", in_out="INPUT",socket_type='NodeSocketFloat')
        self.node_group.interface.new_socket(name='Grid', in_out="INPUT",socket_type='NodeSocketBool')
        ...
        self.node_group.interface.new_socket(name='Line thickness', in_out="INPUT",socket_type='NodeSocketFloat')
        links.new(inputnode.outputs[2], scale_node.inputs.get('Line thickness'))
        self.object.modifiers[-1][self.node_group.interface.items_tree[-1].identifier] =1.0
        ...
        for ax_input in axes_select.inputs:
            self.node_group.interface.new_socket(name=ax_input.name, in_out="INPUT",socket_type='NodeSocketBool')
            links.new(inputnode.outputs.get(ax_input.name), ax_input)
            self.object.modifiers[-1][self.node_group.interface.items_tree[-1].identifier] = True
        
        self.node_group.interface.new_socket("Geometry",in_out="OUTPUT", socket_type='NodeSocketGeometry')
